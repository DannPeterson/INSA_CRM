...
export class DebtsComponent implements OnInit, AfterViewInit  {
  ...
  ngAfterViewInit(): void {
    this.partsDataSource.sortingDataAccessor = (item, property) => {
      switch(property) {
        ...
        case 'agent': return item.policy.agent?.prefix;
        case 'startDate': return item.policy.startDate;
        case 'endDate': return item.policy.endDate;
        case 'partDate': return item.date;
        case 'maxPaymentDay': return item.invoice?.maxDate;
  ...
  // DEBTS SPECIFIC
  getDebtParts(): void {
    this.policyPartService.findAllDebts().subscribe(
      data => {
        this.partsDataSource.data = PolicyPart.fromJsonList(data);
      }
    );
  }
  ...
  updateExistingModalPolicy(fromInvoiceTab: boolean) {
    this.policyService.update(this.modalPolicy.id, this.modalPolicy).subscribe(
      (updatedPolicy) => {
        const index = this.partsDataSource.data.findIndex(part => part.policy.id === updatedPolicy.id);
        if (index !== -1) {
          this.saveModalPolicyParts();
          this.partsDataSource.data[index].policy = Policy.fromJson(updatedPolicy);
          if(!fromInvoiceTab){
            this.modalRef.hide();
          }
          this.partsDataSource.data = [...this.partsDataSource.data];
        } else {
  ...
  fetchPolicy(id: number): void {
    this.policyService.getById(id).subscribe(
      (policy) => {
        const index = this.partsDataSource.data.findIndex(part => part.policy.id === id);
        if (index !== -1) {
          this.partsDataSource.data[index].policy = Policy.fromJson(policy);
          this.partsDataSource.data = [...this.partsDataSource.data];
        }
      }
    );
  ...
  onClientSave() {
    ...
    this.clientService.save(this.selectedClient).subscribe(
      (response: Client) => {
        this.partsDataSource.data.forEach(part => {
