...
@Component({
  selector: 'app-insurer-report-control',
  templateUrl: './insurer-report-control.component.html',
  styleUrls: ['./insurer-report-control.component.css']
})
export class InsurerReportControlComponent implements OnInit {
  @ViewChild('partsTable') partsTable: MatTable<any>;
  displayedColumns: string[] = ['m', 'policyType', 'client', 'object', 'insurer', 'policy', 'start', 'end', 'parts', 'policySum', 'paymentDate', 'partSum', 'partSumReal', 'space', 'paidDate', 'invoice', 'dateConfirmed'];
  ...
  searchPolicy = new FormControl(null);
  ...
  // INSURER REPORT CONTROL SPECIFIC
  calculateSums(): void {
    this.paymentsSum = 0;
  ...
  setDate(): void {
    ...
    this.searchDate.setValue(endOfMonth);
  }
  ...
  getParts(type: number) {
    ...
    const date: Date = this.searchDate.value;
    const client: string = this.searchClient.value || null;
    const policy: string = this.searchPolicy.value || null;

    this.policyPartService.searchInsurerControlParts(insurer, policyType, date, client, policy, type).subscribe(
      data => {
  ...
  onPartDateDbClick(part: PolicyPart) {
    // find policy part in policyParts
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.datePaid = new Date(part.date);
      part.sumReal = part.sum;
      part.dateConfirmed = new Date();
      this.updatePart(part);
    }
  }

  onConfirmedDateDbClick(part: PolicyPart) {
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.dateConfirmed = new Date();
  ...
  onDatePaidDbClick(part: PolicyPart) {
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.datePaid = new Date(part.date);
  ...
  onSumRealDbClick(part: PolicyPart) {
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.sumReal = part.sum;
  ...
  onPartSumRealChange(part: PolicyPart, sumReal: number) {
    // find policy part in policyParts
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.sumReal = sumReal;
      this.updatePart(part);
    }
  }

  onPartDatePaidChange(part: PolicyPart, datePaid: Date) {
    // find policy part in policyParts
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.datePaid = datePaid;
      this.updatePart(part);
    }
  }

  onPartDateConfirmedChange(part: PolicyPart, dateConfirmed: Date) {
    // find policy part in policyParts
    const index = this.policyParts.findIndex(p => p.id === part.id);
    if (index > -1) {
      this.policyParts[index] = part;
      part.dateConfirmed = dateConfirmed;
      this.updatePart(part);
    }
  }

  updatePart(part: PolicyPart) {
    this.policyPartService.update(part.id, part).subscribe(
      data => {
      },
      error => {
        this.sendNotification(NotificationType.ERROR, error.error.message);
  ...
  validateAndProcessInput(event, part: PolicyPart): void {
    const inputValue = event.target.value;
    if (inputValue === '') {
      event.target.classList.remove('error-input'); // Удаляем стиль ошибки, если он был
      return;
    }
    const regex = /^\d+(\.\d{1,2})?$/; // Регулярное выражение для проверки числа

    // Проверяем, соответствует ли введенное значение регулярному выражению
    if (regex.test(inputValue)) {
      event.target.classList.remove('error-input'); // Удаляем стиль ошибки, если он был
      this.onPartSumRealChange(part, inputValue);
      // Здесь можно обработать введенное число, например, отправить его на сервер
    } else {
      event.target.classList.add('error-input'); // Добавляем стиль ошибки
      this.sendNotification(NotificationType.ERROR, 'Неверный формат числа, данные не будут обновлены');
    }
  }

  validateDatePaid(event, part: PolicyPart) {
    const inputValue = event.target.value;
    if (inputValue === '') {
      event.target.classList.remove('error-input'); // Удаляем стиль ошибки, если он был
      return;
    ...
    if (regex.test(inputValue)) {
      ...
      this.onPartDatePaidChange(part, validDate);
      // Введенная дата соответствует формату, проводим дополнительные действия, если требуется
    } else {
      event.target.classList.add('error-input'); // Добавляем стиль ошибки
      this.sendNotification(NotificationType.ERROR, 'Неверный формат даты, данные не будут обновлены');
      // Введенная дата не соответствует формату, отображаем сообщение об ошибке или предпринимаем другие действия
    }
  }

  validateDateConfirmed(event, part: PolicyPart) {
    const inputValue = event.target.value;
    if (inputValue === '') {
      event.target.classList.remove('error-input'); // Удаляем стиль ошибки, если он был
      return;
    ...
    if (regex.test(inputValue)) {
      ...
      const [day, month, year] = inputValue.split(".");
      const validDate = new Date(`${year}-${month}-${day}`);
      this.onPartDateConfirmedChange(part, validDate);
      // Введенная дата соответствует формату, проводим дополнительные действия, если требуется
    } else {
      event.target.classList.add('error-input'); // Добавляем стиль ошибки
      this.sendNotification(NotificationType.ERROR, 'Неверный формат даты, данные не будут обновлены');
