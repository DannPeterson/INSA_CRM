...
export class AgentReportComponent implements OnInit {
  ...
  displayedColumns: string[] = ['m', 'policyType', 'client', 'object', 'insurer', 'policy', 'start', 'end', 'parts', 'sum', 'paymentDate', 'paid', 'datePaid', 'invoice', 'percent', 'provision'];
  ...
  // SEARCH
  searchAgent = new FormControl(null);
    ...
    this.subscriptions.push(
      this.agentService.findAll().subscribe(
        (response: Agent[]) => {
          ...
          this.searchAgent.setValue(this.agents[0]);
        }
      )
    );
  }
  ...
  getPolicyParts(payment: number) {
    this.policyPartService.searchAgentParts(this.searchAgent.value, this.searchDateStart.value, this.searchDateEnd.value, payment).subscribe(
      data => {
