...
@Component({
  selector: 'app-reminder-payments',
  templateUrl: './reminder-payments.component.html',
  styleUrls: ['./reminder-payments.component.css']
})
export class ReminderPaymentsComponent implements OnInit {
  @ViewChild('partsTable') partsTable: MatTable<any>;
  displayedColumns: string[] = ['menu', 'policyNumber', 'insurer', 'client', 'object', 'start', 'end', 'policySum', 'parts', 'part', 'partSum', 'partStart', 'reminder'];
  ...
  // POLICY PARTS SPECIFIC
  getPolicyParts(): void {
    this.policyPartService.searchReminderParts(this.pageIndex, this.pageSize).subscribe(
      (response) => {
        this.policyParts = PolicyPart.fromJsonList(response.content);
        this.totalPolicyParts = response.totalElements;
      }
    );
  }
  ...
  updatePolicyPartFromTable(policyPart: PolicyPart): void {
    this.policyPartService.update(policyPart.id, policyPart).subscribe(
      (updatedPolicyPart: PolicyPart) => {
        const index = this.policyParts.findIndex(item => item.id === updatedPolicyPart.id);
        if (index > -1) {
          this.policyParts[index] = updatedPolicyPart;
        } else {
  ...
  onPartReminderChange(policyPart: PolicyPart): void {
    policyPart.reminder = !policyPart.reminder;
    this.updatePolicyPartFromTable(policyPart);
  }
