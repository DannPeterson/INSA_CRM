...
public class Migration {
    ...
    private final String SQLITE_URL = "jdbc:sqlite:C:\\Users\\danii\\Desktop\\INSA_CRM\\01-backend_db_migration\\insaDB.db";
    ...
    public void migrate() throws SQLException {
        ...
        getPercents();
        System.out.println("PERCENTS DONE");
        getPolicies();
    ...
    private void getPercents() throws SQLException {
        try (
                Connection connection = DriverManager.getConnection(SQLITE_URL);
                Statement statement = connection.createStatement();
                ResultSet resultSet = statement.executeQuery("SELECT * FROM policy_type");
            ...
            while (resultSet.next()) {
                PolicyType policyType = policyTypeService.findById(resultSet.getLong("_id"));

                if (policyType == null) {
                    continue;
                }
                for (long i = 1; i <= 8; i++) {
                    Insurer insurer = insurerService.findById(i);
                    Double percent = resultSet.getDouble("insurer" + i + "perc");

                    InsurerTypeProfitPercent insurerTypeProfitPercent = new InsurerTypeProfitPercent();
                    insurerTypeProfitPercent.setInsurer(insurer);
                    insurerTypeProfitPercent.setPolicyType(policyType);
                    insurerTypeProfitPercent.setPercent(BigDecimal.valueOf(percent));
                    insurerTypeProfitPercentService.save(insurerTypeProfitPercent);
                }
            }
        }
    }
    ...
    private void getPolicies() throws SQLException {
        try (
            ...
            while (resultSet.next()) {
                ...
                if (resultSet.getString("number").equals("4903629678")) {
                ...
                String dateConcl = resultSet.getString("date_concl");
                if (dateConcl != null) {
                    policy.setConclusionDate(LocalDate.parse(dateConcl, formatter));
                } else {
                    if(dateStart != null) {
                        policy.setConclusionDate(LocalDate.parse(dateStart, formatter));
                    }
                }
                ...
                try{
                    System.out.println(formatter.format(policy.getConclusionDate()));
                } catch (Exception e) {
    ...
    private PolicyPart getPart(ResultSet resultSet, Policy policy, int part) throws SQLException {
        ...
        if (part_date != null) {
            ...
            if (policy.getStopDate() != null && policyPart.getDate().isAfter(policy.getStopDate())) {
                policyPart.setReminder(false);
            }
